...
ImgParams.N_pixPerXblock  = 51; %pixels per X block
ImgParams.N_pixPerZblock  = 51; %pixels per Z block
...
Ublocks = zeros( FMC.Probe.Nelements , ImgParams.NsingvalsRetain , kfmax-kfmin+1 , ImgParams.N_Xblocks*ImgParams.N_Zblocks , 'single' );
for i_block = 1 : ImgParams.N_Xblocks*ImgParams.N_Zblocks
    for k = 1 : kfmax-kfmin+1
        N = FMC.Probe.Nelements;
        Ublocks( : , : , k , i_block ) = single( U( 1+(k-1)*N : k*N , : , i_block ) );
    end
end
...
A_fkU    = zeros( FMC.Probe.Nelements                               , ImgParams.NsingvalsRetain                    , kfmax-kfmin+1                           , ImgParams.N_Xblocks*ImgParams.N_Zblocks , 'single' );
UtA_fkU1 = zeros( ImgParams.NsingvalsRetain                         , ImgParams.NsingvalsRetain                    , kfmax-kfmin+1                           , ImgParams.N_Xblocks*ImgParams.N_Zblocks , 'single' );
...
A_fkU    = pagemtimes( A_fk , Ublocks );
UtA_fkU1 = pagemtimes( Ublocks , 'transpose' , A_fkU , 'none' );
UtA_fkU  = squeeze( sum( UtA_fkU1 , 3 ) );
